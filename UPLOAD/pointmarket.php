...
if ($_POST['buypoints']){
	...
	$result = mysqli_query($conn, "SELECT * FROM `pointsmarket` WHERE `id`='".$_POST['points_id']."'");
	...
	if ($worked['owner'] == $user_class->id) {
		...
		$result = mysqli_query($conn, "UPDATE `grpgusers` SET `points` = '".$newpoints."' WHERE `id`='".$user_class->id."'");
		$user_class = new User($_SESSION['id']);
			if ($newpointsinmarket == 0){
				$result = mysqli_query($conn, "DELETE FROM `pointsmarket` WHERE `id`='".$worked['id']."'");
			} else {
				$result = mysqli_query($conn, "UPDATE `pointsmarket` SET `amount` = '".$newpointsinmarket."' WHERE `id`='".$worked['id']."'");
		}
	include 'footer.php';
	die();
	}
	...
	if($_POST['amount'] >= 1 && $_POST['amount'] <= $amount && $totalcost <= $user_class->money){
		...
		$result = mysqli_query($conn, "UPDATE `grpgusers` SET `money` = '".$newmoney."', `points` = '".$newpoints."' WHERE `id`='".$user_class->id."'");
		$newmoney = $user_points->money + $totalcost;
		$result = mysqli_query($conn, "UPDATE `grpgusers` SET `money` = '".$newmoney."' WHERE `id`='".$user_points->id."'");
		$user_class = new User($_SESSION['id']);
			if ($newpointsinmarket == 0){
				$result = mysqli_query($conn, "DELETE FROM `pointsmarket` WHERE `id`='".$worked['id']."'");
			} else {
...
if ($_POST['addpoints']){
	...
	if($_POST['amount'] >= 1 && $_POST['amount'] <= $user_class->points && $_POST['price'] >= 1){
		...
		$result= mysqli_query($conn, "INSERT INTO `pointsmarket` (owner, amount, price)"."VALUES ('$user_class->id', '$_POST[amount]', '$_POST[price]')");
		$newpoints = $user_class->points - $_POST['amount'];
		$result = mysqli_query($conn, "UPDATE `grpgusers` SET `points` = '".$newpoints."' WHERE `id`='".$user_class->id."'");
		$user_class = new User($_SESSION['id']);
	}
