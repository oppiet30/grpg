...
if ($_POST['takebet'] != ""){
	...
	$result = mysqli_query($conn, "SELECT * FROM `5050game` WHERE `id`='".$_POST['bet_id']."'");
	...
	if($amount <= $user_class->money){
		...
		$result = mysqli_query($conn, "DELETE FROM `5050game` WHERE `id`='".$worked['id']."'");
...
if ($_POST['makebet']){
	...
	if($_POST['amount'] >= 1000 && $_POST['amount'] <= $user_class->money){
		...
		$result= mysqli_query($conn, "INSERT INTO `5050game` (owner, amount)"."VALUES ('$user_class->id', '$_POST[amount]')");
...
<tr><td class="contentcontent">
<form method='post'>
Amount of money to bid. $<input type='text' name='amount' size='10' maxlength='20' value='<?php echo $user_class->money ?>'> (minimum of $1000 bet)<br>
