...
if ($_GET['sell'] != "") {
	...
	$newsql = mysqli_query($conn, "UPDATE `grpgusers` SET `money` = '".$newmoney."', `marijuana`='0' WHERE `id`= '".$user_class->id."'");
...
if ($_GET['buy'] != "") {
	...
	if($_GET['buy'] == "potseeds"){
		...
		$newsql = mysqli_query($conn, "UPDATE `grpgusers` SET `potseeds` = '".$newamount."' WHERE `id`= '".$user_class->id."'");
	}
}
