...
if (isset($_POST['submit'])) {
  ...
  $email = $_POST["email"];
  $checkuser = mysqli_query($conn, "SELECT * FROM `grpgusers` WHERE `username`='$username'");
...
//  if (!eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
  ...
  //insert the values
  if (!isset($message)){
    $result= mysqli_query($conn, "INSERT INTO `grpgusers` (ip, username, password, email, signuptime, lastactive)".
	...
	if ($_POST['referer'] != ""){
	$result= mysqli_query($conn, "INSERT INTO `referrals` (`when`, `referrer`, `referred`)".
