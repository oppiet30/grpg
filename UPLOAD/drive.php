...
$result = mysqli_query($conn, "SELECT * FROM `cars` WHERE `userid`='".$user_class->id."'");
...
if ($_GET['go'] != "") {
	...
	$result = mysqli_query($conn, "SELECT * FROM `cities` WHERE `id`='".$_GET['go']."'");
	...
	if (!isset($error)){
		...
		$result = mysqli_query($conn, "UPDATE `grpgusers` SET `city` = '".$_GET['go']."', `money` = '".$newmoney."' WHERE `id` = '".$user_class->id."'");
